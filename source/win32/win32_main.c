...
WinMain(HINSTANCE instance, HINSTANCE prev_instance, LPSTR lp_cmd_line, int n_show_cmd)
{
    ...
    // NOTE(tbt): direct input stuff
    LPDIRECTINPUTDEVICE8 direct_input_mouse;
    {
        IDirectInput8 *direct_input;
        
        HRESULT result;
        result = DirectInput8Create(global_instance_handle,
                                    DIRECTINPUT_VERSION,
                                    &IID_IDirectInput8A,
                                    &direct_input,
                                    NULL);
        if(DI_OK == result)
        {
            direct_input->lpVtbl->CreateDevice(direct_input,
                                               &GUID_SysMouse,
                                               &direct_input_mouse,
                                               NULL);
            direct_input_mouse->lpVtbl->SetDataFormat(direct_input_mouse, &c_dfDIMouse);
            direct_input_mouse->lpVtbl->Acquire(direct_input_mouse);
        }
    }
    ...
    while(!global_os.quit)
    {
        ...
        // NOTE(tbt): show or hide cursor
        {
            local_persist prev_show_cursor = 1;
            if(global_os.show_cursor != prev_show_cursor)
            {
                if(global_os.show_cursor)
                {
                ...
                prev_show_cursor = global_os.show_cursor;
            }
        }
        ...
        OS_BeginFrame();
        {
            ...
            direct_input_mouse->lpVtbl->GetDeviceState(direct_input_mouse, sizeof(mouse_state), &mouse_state);
            global_os.mouse_delta = v2(mouse_state.lX, mouse_state.lY);
        }
